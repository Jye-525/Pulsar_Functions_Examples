...
public class DataStructureSerde {
	...
	static public DataStructure deserialize(byte[] input) {
		...
		DataStructure dataStructure = new DataStructure(fields[0], Long.parseLong(fields[1]), Long.parseLong(fields[2]), Long.parseLong(fields[3]));
		return dataStructure;
	}
	
	static public byte[] serialize(DataStructure input) {
		String resultString = String.format("%s|%d|%d|%d", input.getSentence(), input.getStartTime(), input.getEndTime(), input.getMsgLength());
